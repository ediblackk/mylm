//! Worker LLM settings menu loop

use anyhow::Result;

use crate::tui::hub::choices::{
    AgenticSettingsChoice, ContextSettingsChoice, PaCoReSubSettingsChoice, WorkerLLMSettingsChoice,
};
use crate::tui::hub::handlers;
use dialoguer::Select;
use mylm_core::config::Config;

/// Worker LLM settings loop
pub async fn run(config: &mut Config) -> Result<()> {
    loop {
        match show_worker_llm_settings_menu()? {
            WorkerLLMSettingsChoice::SelectModel => {
                handlers::handle_select_worker_model(config).await?;
            }
            WorkerLLMSettingsChoice::ContextSettings => {
                run_context_settings(config, false).await?;
            }
            WorkerLLMSettingsChoice::AgenticSettings => {
                run_agentic_settings(config, false).await?;
            }
...
fn show_worker_llm_settings_menu() -> Result<WorkerLLMSettingsChoice> {
    use console::Style;
...
async fn run_context_settings(config: &mut Config, is_main: bool) -> Result<()> {
    use crate::tui::hub::choices::ContextSettingsChoice;
    use crate::tui::hub::handlers;
