...
pub struct TerminalDelegate {
    event_tx: mpsc::UnboundedSender<crate::tui::app::state::TuiEvent>,
...
impl TerminalExecutor for TerminalDelegate {
    async fn execute_command(&self, cmd: String, timeout: Option<std::time::Duration>) -> Result<String, String> {
        ...
        if let Err(e) = self.event_tx.send(crate::tui::app::state::TuiEvent::ExecuteTerminalCommand(cmd.clone(), tx)) {
    ...
    async fn get_screen(&self) -> Result<String, String> {
        ...
        if let Err(e) = self.event_tx.send(crate::tui::app::state::TuiEvent::GetTerminalScreen(tx)) {
