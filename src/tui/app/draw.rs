...
use crate::tui::app::types::{AppState, Focus, JobStatus};
...
fn draw_chat_history(f: &mut Frame, app: &App, area: Rect) {
    ...
    for msg in &app.chat_history {
        let (prefix, style) = match msg.message.role {
        ...
        let content = format!("{}{}", prefix, msg.message.content);
...
fn draw_help_overlay(f: &mut Frame, _app: &App, area: Rect) {
    let help_text = crate::tui::app::help::HelpSystem::generate_help_text(_app, None);
