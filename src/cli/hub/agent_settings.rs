...
use mylm_core::config::v2::AgentPermissions;
...
pub fn show_rate_limit_settings_menu(config: &Config) -> Result<RateLimitSettingsChoice> {
    ...
    println!();
    println!("  These limits control how many requests per minute");
    println!("  are made to the LLM provider.");
    println!();
    println!("  Main Agent:  Requests from the main agent");
    println!("  Workers:     Shared pool for all background jobs");
    println!();
