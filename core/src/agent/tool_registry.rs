...
pub struct ToolRegistry {
    tools: std::sync::Arc<RwLock<std::collections::HashMap<String, std::sync::Arc<dyn crate::agent::tool::Tool>>>>,
...
impl ToolRegistry {
    ...
    pub async fn get_tool_arc(&self, name: &str) -> Option<std::sync::Arc<dyn crate::agent::tool::Tool>> {
    ...
    pub async fn register_tool(&self, tool: Box<dyn crate::agent::tool::Tool>) -> Result<()> {
    ...
    pub async fn register_tool_arc(&self, tool: std::sync::Arc<dyn crate::agent::tool::Tool>) -> Result<()> {
        let mut tools = self.tools.write().await;
        let tool_name = tool.name().to_string();
    ...
    pub async fn unregister_tool(&self, name: &str) -> Result<Option<std::sync::Arc<dyn crate::agent::tool::Tool>>> {
        let mut tools = self.tools.write().await;
    ...
    pub async fn get_tool(&self, name: &str) -> Option<std::sync::Arc<dyn crate::agent::tool::Tool>> {
        // Check if tool is disabled
        let disabled = self.disabled_tools.read().await;
        if disabled.contains_key(name) {
            return None;
    ...
    pub async fn get_all_tools(&self) -> Vec<std::sync::Arc<dyn crate::agent::tool::Tool>> {
        let tools = self.tools.read().await;
        let disabled = self.disabled_tools.read().await;
    ...
    pub async fn get_tool_kind(&self, name: &str) -> Option<crate::agent::tool::ToolKind> {
        let disabled = self.disabled_tools.read().await;
        if disabled.contains_key(name) {
            return None;
        }
    ...
    pub async fn execute_tool(&self, name: &str, args: &str) -> Result<crate::agent::tool::ToolOutput, String> {
