...
use crate::agent::v2::jobs::{JobRegistry, AgentType, JobStatus};
use crate::agent::prompt::PromptBuilder;
use crate::agent::tools::worker_shell::{WorkerShellTool, WorkerShellPermissions, EscalationRequest, EscalationResponse};
use crate::agent::event_bus::CoreEvent;
use crate::config::v2::types::{WorkerShellConfig, EscalationMode};
use crate::config::v2::ProviderConfig;
...
pub type SharedScratchpad = Arc<TokioRwLock<crate::agent::tools::scratchpad::StructuredScratchpad>>;
...
pub struct DelegateTool {
    ...
    event_bus: Option<Arc<crate::agent::event_bus::EventBus>>,
    ...
    async fn call(&self, args: &str) -> Result<ToolOutput, Box<dyn StdError + Send + Sync>> {
        ...
        for worker in &args.workers {
            for job in &active_jobs {
                if job.status == crate::agent::v2::jobs::JobStatus::Running {
        ...
        let scratchpad = Arc::new(TokioRwLock::new(
            crate::agent::tools::scratchpad::StructuredScratchpad::new()
        ));
...
impl DelegateTool {
    async fn spawn_worker(
        ...
        tokio::spawn(async move {
            ...
            // Create agent
            let agent_config = crate::agent::v2::AgentV2Config {
                client,
                scribe,
                tools,
            ...
            let bus = event_bus.unwrap_or_else(|| Arc::new(crate::agent::event_bus::EventBus::new()));
