//! Stall Scheduler - Handle stalled worker jobs
//!
...
/// Scheduler for handling stalled worker jobs.
///
...
impl StallScheduler {
    /// Create a new stall scheduler and start background event listener.
            ...
            match receiver.recv().await {
                Ok(CommonboxEvent::JobStalled { job_id, reason }) => {
                    ...
                    let stalled_job = StalledJob {
                        job_id,
                        agent_id: crate::agent::AgentId::main(), // placeholder
                        reason,
...
mod tests {
    ...
    async fn test_stall_scheduler_creation() {
        ...
        let scheduler = StallScheduler::new(commonbox);
