...
use crate::agent::contract::session::{Session, UserInput, OutputEvent};
use crate::agent::runtime::tools::scratchpad::SharedScratchpad;
...
pub async fn run_worker_session(
    ...
    commonbox: Arc<Commonbox>,
    scratchpad: SharedScratchpad,
    mut session: crate::agent::contract::session::AgencySession<
        crate::agent::cognition::kernel_adapter::CognitiveEngineAdapter<crate::agent::cognition::LLMBasedEngine>,
        crate::agent::runtime::contract_runtime::ContractRuntime,
        crate::agent::runtime::impls::InMemoryTransport,
    >,
    id_to_job: Arc<RwLock<HashMap<String, JobId>>>,
    ...
    // Run session using the Session trait
    ...
    match session.run().await {
        Ok(result) => {
            ...
            loop {
                ...
                for query in queries {
                    ...
                    // Process query (simplified - in real impl, would run another session)
        ...
        Err(e) => {
            // Emit error/failed event for TUI
            if let Some(ref tx) = parent_output_tx {
                let _ = tx.send(OutputEvent::Error {
                    message: format!("Worker {} failed: {}", config.id, e),
                });
            }
