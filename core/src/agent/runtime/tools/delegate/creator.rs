...
use crate::agent::contract::session::{AgencySession, OutputEvent};
use crate::agent::cognition::kernel_adapter::CognitiveEngineAdapter;
use crate::agent::cognition::LLMBasedEngine;
use crate::agent::runtime::contract_runtime::ContractRuntime;
use crate::agent::runtime::impls::InMemoryTransport;
...
pub async fn create_worker_session(
    ...
    let session = match ctx.factory.create_worker_session(&config.id, worker_config).await {
        Ok(s) => {
...
pub fn emit_worker_spawned_event(
    output_tx: &Option<crate::agent::runtime::contract_runtime::OutputSender>,
    worker_id: WorkerId,
    objective: String,
) {
...
pub async fn initialize_scratchpad_entry(
    scratchpad: &crate::agent::runtime::tools::scratchpad::SharedScratchpad,
    config: &WorkerConfig,
) {
