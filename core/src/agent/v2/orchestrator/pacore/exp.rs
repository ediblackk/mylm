//! Core PaCoRe experiment orchestration for multi-round LLM inference.
//!
//! Implements the parallel consensus reasoning algorithm: running multiple
//! LLM calls in parallel across rounds, synthesizing results, and streaming
//! final responses with progress callbacks.
//!
//! # Main Types
//! - `Exp`: Experiment orchestrator with configurable rounds and concurrency
//! - `ProcessedResult`: Final result with all round data
//! - `RoundResult`: Results from a single inference round
//! - `PaCoReProgressEvent`: Progress notifications for UI feedback
//! - `BatchItem`/`BatchResult`: Batch processing types
...
impl Exp {
        ...
        for (round_idx, &num_calls) in self.num_responses_per_round.iter().enumerate() {
            let current_messages = if round_idx == 0 {
                ...
                let prev_answers: Vec<String> = all_rounds.last()
                    .expect("all_rounds should not be empty when round_idx > 0")
                    .responses
                    .iter()
    ...
    async fn run_parallel_calls(
        ...
        let num_batches = num_calls.div_ceil(self.max_concurrent);
    ...
    fn update_last_user_message(&self, messages: &mut [Message], new_content: String) -> Result<(), Error> {
        if let Some(m) = messages.iter_mut().rfind(|m| m.role == "user") {
