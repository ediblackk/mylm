...
impl AgentV2 {
    ...
    pub fn new_with_config(config: AgentV2Config) -> Self {
        ...
        let job_registry = config.job_registry.unwrap_or_default();
    ...
    pub async fn step(&mut self, observation: Option<String>) -> Result<AgentDecision, Box<dyn StdError + Send + Sync>> {
        ...
        if scratchpad_size > crate::agent::tools::scratchpad::SCRATCHPAD_WARNING_SIZE {
            let warning = format!(
                ...
                scratchpad_size,
                crate::agent::tools::scratchpad::SCRATCHPAD_WARNING_SIZE,
                crate::agent::tools::scratchpad::SCRATCHPAD_CRITICAL_SIZE
            );
        ...
        if scratchpad_size > crate::agent::tools::scratchpad::SCRATCHPAD_CRITICAL_SIZE {
        ...
        if let Some(warning) = self.context_manager.preflight_check(None) {
            ...
            if let Some(event_bus) = &self.event_bus {
                event_bus.publish(crate::agent::event_bus::CoreEvent::StatusUpdate {
