...
use super::{IntentId, IntentNode, Intent, AgentError};
...
impl IntentGraph {
        ...
        for node in self.nodes.values() {
            for dep in &node.dependencies {
                if !self.nodes.contains_key(dep) {
                    return Err(AgentError::UnknownDependency(*dep));
                }
            }
        }
    ...
    pub fn topological_order(&mut self) -> Result<&[IntentId], AgentError> {
        if self.topological_order.is_none() {
            let order = self.compute_topological_order()
                .map_err(|cycle| AgentError::CyclicDependency(cycle))?;
...
impl IntentGraphBuilder {
    ...
    /// Build and validate
    pub fn build_validated(self) -> Result<IntentGraph, AgentError> {
    ...
    pub fn node_mut(&mut self, id: IntentId) -> Option<&mut super::IntentNode> {
...
mod tests {
    ...
    fn test_single_node() {
        ...
        graph.add(IntentNode::new(IntentId::new(1), Intent::Halt(ExitReason::Completed)));
