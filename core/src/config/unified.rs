...
//!
//! Main configuration system that combines profiles, providers, and app settings
//! into a single TOML configuration file.
...
impl Config {
    ...
    pub fn load_or_default() -> Self {
        ...
        if let Some(config_dir) = dirs::config_dir() {
            ...
            if legacy_path.exists() {
                if let Ok(legacy_config) = crate::config::legacy::ConfigV2::load() {
    ...
    fn from_legacy(legacy: &crate::config::legacy::ConfigV2) -> Self {
        use crate::config::base::Provider;
