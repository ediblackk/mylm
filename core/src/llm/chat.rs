...
pub struct ChatMessage {
    ...
    /// This must be preserved in assistant messages when sending back to the API
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl ChatMessage {
    ...
    /// Create a new assistant message with reasoning content (for thinking-enabled models)
...
impl ChatResponse {
    ...
    pub fn extract_memories(&self) -> Vec<String> {
        ...
        let re = Regex::new(r"\[MEMORY:\s*(.*?)]").expect("Valid MEMORY regex pattern");
    ...
    pub fn extract_searches(&self) -> Vec<String> {
        let content = self.content();
        let re = Regex::new(r"\[SEARCH:\s*(.*?)]").expect("Valid SEARCH regex pattern");
        re.captures_iter(&content)
            .map(|cap| cap[1].trim().to_string())
            .filter(|s| !s.is_empty())
            .collect()
    ...
    pub fn display_content(&self) -> String {
        let content = self.content();
        let re_mem = Regex::new(r"\[MEMORY:\s*.*?]\n?").expect("Valid MEMORY regex pattern");
        let re_search = Regex::new(r"\[SEARCH:\s*.*?]\n?").expect("Valid SEARCH regex pattern");
