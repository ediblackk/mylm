...
pub enum AgentWrapper {
    V1(Arc<Mutex<crate::agent_old::Agent>>),
    V2(Arc<Mutex<crate::agent_old::v2::AgentV2>>),
}

impl AgentWrapper {
    /// Create a new V1 wrapper
    pub fn new_v1(agent: crate::agent_old::Agent) -> Self {
        Self::V1(Arc::new(Mutex::new(agent)))
    }
    
    /// Create a new V2 wrapper
    pub fn new_v2(agent: crate::agent_old::v2::AgentV2) -> Self {
    ...
    pub fn as_v1_arc(&self) -> Option<Arc<Mutex<crate::agent_old::Agent>>> {
        match self {
    ...
    pub fn as_v2_arc(&self) -> Option<Arc<Mutex<crate::agent_old::v2::AgentV2>>> {
        match self {
    ...
    pub async fn job_registry(&self) -> Arc<JobRegistry> {
        match self {
            AgentWrapper::V1(a) => {
                let guard = a.lock().await;
                Arc::new(guard.job_registry.clone())
            }
            AgentWrapper::V2(a) => {
                let guard = a.lock().await;
    ...
    /// Get permissions
    pub async fn permissions(&self) -> Option<crate::config::types::AgentPermissions> {
        match self {
            AgentWrapper::V1(a) => {
                let guard = a.lock().await;
    ...
    pub async fn tool_registry(&self) -> Option<crate::agent_old::ToolRegistry> {
        match self {
            AgentWrapper::V1(a) => {
                let guard = a.lock().await;
    ...
    pub async fn tools(&self) -> Option<std::collections::HashMap<String, Arc<dyn crate::agent_old::Tool>>> {
        match self {
    ...
    pub async fn step_v2(&self, observation: Option<String>) -> anyhow::Result<crate::agent_old::v2::protocol::AgentDecision> {
        match self {
            AgentWrapper::V1(_) => {
    ...
    pub async fn get_tools(&self) -> Option<std::collections::HashMap<String, Arc<dyn crate::agent_old::Tool>>> {
        match self {
            AgentWrapper::V1(_) => None,
            AgentWrapper::V2(a) => {
                let guard = a.lock().await;
