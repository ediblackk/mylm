...
use crate::agent_old::event_bus::{EventBus, CoreEvent};
use crate::agent_old::tools::StructuredScratchpad;
use crate::agent_old::v2::AgentV2;
use crate::agent_old::v2::AgentV2Config;
...
pub struct AgentConfig {
    ...
    pub job_registry: Option<crate::agent_old::v2::jobs::JobRegistry>,
    pub scratchpad: Option<Arc<RwLock<StructuredScratchpad>>>, // Note: tokio::sync::RwLock
    pub disable_memory: bool,
    pub permissions: Option<crate::config::types::AgentPermissions>,
...
use crate::agent_old::protocol::{parse_short_key_action_from_content, parse_react_action, truncate_for_log};
use crate::agent_old::context::{prune_history, build_context_from_memories};
use crate::agent_old::execution::{process_tool_args, is_final_response, build_continue_nudge, build_format_nudge};
...
pub struct Agent {
    ...
    pub tool_registry: crate::agent_old::ToolRegistry,
    pub max_iterations: usize,
    pub system_prompt_prefix: String,
    pub memory_store: Option<Arc<crate::memory::store::VectorStore>>,
    pub categorizer: Option<Arc<MemoryCategorizer>>,
    pub scribe: Option<Arc<crate::memory::scribe::Scribe>>,
    pub job_registry: crate::agent_old::v2::jobs::JobRegistry,
    pub session_id: String,
    pub version: crate::config::AgentVersion,
    pub scratchpad: Option<Arc<RwLock<StructuredScratchpad>>>,
    ...
    // Embedded V2 agent for parallel execution (when version == V2)
    pub v2_agent: Option<AgentV2>,
}

impl Agent {
    /// Create a new Agent with individual parameters (deprecated, use `new_with_config`)
    ...
    pub async fn new_with_iterations(
        ...
        scratchpad: Option<Arc<RwLock<StructuredScratchpad>>>,
        disable_memory: bool,
        permissions: Option<crate::config::types::AgentPermissions>,
    ) -> Self {
        Self::new_with_config(AgentConfig {
            client,
            tools,
    ...
    /// Create a new Agent from a configuration struct
    pub async fn new_with_config(config: AgentConfig) -> Self {
        let tool_registry = crate::agent_old::ToolRegistry::new();
        ...
        let job_registry = config.job_registry.clone().unwrap_or_default();
        
        // Initialize Scribe for V2 if store is available
        let scribe = if config.version == crate::config::AgentVersion::V2 {
            if let Some(store) = &config.memory_store {
                let journal = Arc::new(Mutex::new(crate::memory::journal::Journal::new()
        ...
        // Create V2 agent if version is V2
        let v2_agent = if config.version == crate::config::AgentVersion::V2 {
            // Ensure required dependencies
            let store = config.memory_store.as_ref().expect("V2 requires memory_store");
            let journal = Arc::new(Mutex::new(
                Journal::new().expect("Failed to create journal for V2 scribe")
            ));
            ...
            let v2_config = AgentV2Config {
                client: config.client.clone(),
                scribe,
                tools,
                system_prompt_prefix: config.system_prompt_prefix.clone(),
                max_iterations: config.max_iterations,
                version: config.version,
                memory_store: config.memory_store.clone(),
                categorizer: config.categorizer.clone(),
            ...
            Some(AgentV2::new_with_config(v2_config))
        } else {
            None
        };
    ...
    pub fn has_pending_decision(&self) -> bool {
        if let Some(v2) = &self.v2_agent {
            v2.has_pending_decision()
        } else {
    ...
    pub async fn reset(&mut self, history: Vec<ChatMessage>) {
        if self.version == crate::config::AgentVersion::V2 {
            if let Some(v2) = &mut self.v2_agent {
                v2.reset(history).await;
                return;
            }
        }
    ...
    pub async fn step(&mut self, observation: Option<String>) -> Result<AgentDecision, Box<dyn StdError + Send + Sync>> {
        // If V2, delegate to embedded AgentV2
        if self.version == crate::config::AgentVersion::V2 {
            if let Some(v2) = &mut self.v2_agent {
                let v2_decision = v2.step(observation).await?;
                return Ok(self.convert_v2_decision(v2_decision));
            } else {
                return Err("V2 agent not initialized".into());
            }
        }
        ...
        // 3. Add observation to history if provided (truncated to prevent context explosion)
        const MAX_OBS_CHARS: usize = 8000;
        if let Some(obs) = observation {
            let truncated_obs = if obs.len() > MAX_OBS_CHARS {
                format!("{}...[truncated {} chars]", &obs[..MAX_OBS_CHARS], obs.len() - MAX_OBS_CHARS)
            } else {
                obs
            };
            let preview: String = truncated_obs.chars().take(100).collect();
            crate::info_log!("Agent adding observation to history (len={}): {}", truncated_obs.len(), preview);
            ...
            crate::info_log!("History now has {} messages", self.history.len());
        } else {
            crate::info_log!("Agent step called with NO observation");
        }
        ...
        crate::info_log!("Sending {} history messages to LLM", self.history.len());
        for (i, msg) in self.history.iter().enumerate() {
            crate::debug_log!("  History[{}]: role={:?}, content_len={}", i, msg.role, msg.content.len());
        }
        ...
        crate::info_log!("Calling LLM chat with {} messages", self.history.len());
    ...
    /// Convert V2 AgentDecision to V1 AgentDecision
    fn convert_v2_decision(&self, v2_decision: crate::agent_old::v2::AgentDecision) -> AgentDecision {
        match v2_decision {
            crate::agent_old::v2::AgentDecision::Message(msg, usage) => AgentDecision::Message(msg, usage),
            crate::agent_old::v2::AgentDecision::Action { tool, args, kind } => AgentDecision::Action {
                tool,
                args,
                kind,
            },
            crate::agent_old::v2::AgentDecision::MalformedAction(error) => AgentDecision::MalformedAction(error),
            crate::agent_old::v2::AgentDecision::Error(e) => AgentDecision::Error(e),
            crate::agent_old::v2::AgentDecision::Stall { reason, tool_failures } => AgentDecision::Stall { reason, tool_failures },
        }
    }
    ...
    async fn handle_short_key_decision(
        &mut self,
        content: String,
        sk_action: crate::agent_old::protocol::ShortKeyAction,
    ) -> Result<AgentDecision, Box<dyn StdError + Send + Sync>> {
    ...
    async fn handle_native_tool_call(
        ...
        let tool_calls = response.choices[0].message.tool_calls.as_ref()
            .ok_or_else(|| Box::new(std::io::Error::new(std::io::ErrorKind::InvalidData, "Missing tool_calls")) as Box<dyn StdError + Send + Sync>)?;
        ...
        let tool_name = crate::agent_old::tool_registry::normalize_tool_name(raw_name).to_string();
    ...
    async fn handle_react_decision(
        &mut self,
        content: String,
        react_action: crate::agent_old::protocol::ReActAction,
    ) -> Result<AgentDecision, Box<dyn StdError + Send + Sync>> {
    ...
    pub async fn run(
        ...
        crate::info_log!("Agent::run() called with version={:?}, checking for V2...", self.version);
        if self.version == crate::config::AgentVersion::V2 {
            crate::info_log!("Agent::run() delegating to V2 run_event_driven");
            if let Some(v2) = &mut self.v2_agent {
                // Setup interrupt watcher to convert flag to channel
                ...
                // Ensure approval_rx is Some
                let approval_rx = approval_rx.unwrap_or_else(|| {
                    let (_, rx) = tokio::sync::mpsc::channel(1);
                    rx
                });
                return v2.run_event_driven(history, event_bus, interrupt_rx, approval_rx).await;
            } else {
                return Err("V2 agent not initialized".into());
            }
        }
        crate::info_log!("Agent::run() using V1 sequential mode");

        // 1. Memory Context Injection (if enabled)
        // Memory context injection (if enabled)
        ...
        for _loop_iteration in 1..=max_driver_loops {
            ...
            match self.step(last_observation.take()).await? {
                ...
                AgentDecision::Action { tool, args, kind } => {
                    ...
                    // Handle approval
                    if !auto_approve {
                        match self.handle_approval_flow(&tool, &args, &event_bus, &mut approval_rx).await {
                ...
                AgentDecision::MalformedAction(error) => {
                    ...
                    let _ = event_bus.publish(CoreEvent::StatusUpdate {
                        message: format!("⚠️ {} Retrying ({}/{})", error, retry_count, max_retries)
                    });
    ...
    pub async fn condense_history(
        ...
        crate::agent_old::context::condense_history(history, &self.llm_client).await
    }
    ...
    pub async fn inject_memory_context(&mut self) -> Result<(), Box<dyn StdError + Send + Sync>> {
        ...
        crate::agent_old::context::inject_memory_context(
            &mut self.history,
    ...
    /// Generate the system prompt with available tools and configuration.
    pub async fn generate_system_prompt(&self) -> String {
        ...
        // Build tools HashMap for PromptBuilder
        ...
        let scratchpad_content = if let Some(scratchpad) = &self.scratchpad {
            match scratchpad.try_read() {
                Ok(guard) => guard.to_string(),
                Err(_) => {
                    crate::error_log!("Scratchpad lock busy in generate_system_prompt, returning empty string");
                    String::new()
                }
            }
        } else {
        ...
        let prompt_builder = PromptBuilder::new(
            ...
            tools_map,
            None, // capabilities_context - not used in V1
        );
        
        prompt_builder.build(&scratchpad_content)
    }

    /// Handle the approval flow for tool execution.
    ///
    ...
    pub async fn handle_approval_flow(
        ...
        args: &str,
        event_bus: &Arc<EventBus>,
        approval_rx: &mut Option<tokio::sync::mpsc::Receiver<bool>>,
    ) -> Result<bool, String> {
        // V1 agents don't have a permissions field on the Agent itself for auto-approve
        // The auto_approve flag is handled by the caller (run method), so we pass false here
        crate::agent_old::execution::handle_tool_approval(
            tool,
            args,
            false, // auto_approve - caller already checked this
            event_bus,
            approval_rx,
        ).await
    }
    ...
    pub async fn get_system_prompt(&self) -> String {
        self.generate_system_prompt().await
    }
}
